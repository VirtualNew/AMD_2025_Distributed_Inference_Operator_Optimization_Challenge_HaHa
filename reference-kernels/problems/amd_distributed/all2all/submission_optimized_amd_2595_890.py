...
# ---------------- AMD ROCm Optimized Packed All2All Implementation ----------------
class AMDOptimizedPackedAllToAll:
    ...
    def dispatch(self, dp_x: torch.Tensor, indices: torch.Tensor):
        ...
        # AMD optimization: Align buffer size to larger alignment factor for better memory coalescing
    ...
    def combine(self, out_tokens: torch.Tensor, weights: torch.Tensor, 
        ...
        # Prepare receive buffer
        # AMD optimization: Align buffer size to larger alignment factor
        aligned_total_recv = self._align_dimension(total_recv, self.alignment_factor)
        recv_combined = torch.empty((aligned_total_recv, per_row_len), dtype=torch.float32, device=device)
        recv_combined = self._optimize_memory_layout(recv_combined)
...
def custom_kernel(data: input_t) -> output_t:
    ...
    # Create AMD optimized all-to-all instance
    ata = AMDOptimizedPackedAllToAll(cfg, rank, world_size)
