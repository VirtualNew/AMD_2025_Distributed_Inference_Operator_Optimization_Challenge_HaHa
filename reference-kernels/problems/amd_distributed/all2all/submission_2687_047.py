...
# ---------------- High-Performance Packed All2All Implementation ----------------
class HighPerformancePackedAllToAll:
    ...
    def dispatch(self, dp_x: torch.Tensor, indices: torch.Tensor):
        """
        Dispatch tokens to experts using packed all-to-all for high performance.
        ...
        # Prepare send buffer with packing
        if total_send == 0:
        ...
        aligned_total_recv = self._align_dimension(int(total_recv), self.alignment_factor)
        ...
        if total_send > 0 and total_recv > 0:
            dist.all_to_all_single(
                recv_combined[:int(total_recv)],  # Use only the needed portion
                combined_send,
        ...
        recv_meta_float = recv_combined[:int(total_recv), :self.META_DIM]
        recv_data_float = recv_combined[:int(total_recv), self.META_DIM:]
    ...
    def combine(self, out_tokens: torch.Tensor, weights: torch.Tensor, 
        ...
        """
        Combine expert outputs back to tokens using packed all-to-all for high performance.
        """
        cfg = self.cfg
...
def custom_kernel(data: input_t) -> output_t:
    """
    High-performance packed custom kernel implementation.
    """
    cfg, rank_data, rank, world_size = data
    torch.cuda.set_device(rank)
    device = torch.device('cuda', rank)
    
    # Create high-performance packed all-to-all instance
    ata = HighPerformancePackedAllToAll(cfg, rank, world_size)
