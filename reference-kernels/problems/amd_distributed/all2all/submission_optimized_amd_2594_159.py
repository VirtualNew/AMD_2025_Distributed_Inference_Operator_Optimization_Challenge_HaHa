...
class AMDOptimizedPackedAllToAll:
    ...
    def _align_to_wavefront(self, tensor: torch.Tensor, dim_size: int):
        """
        Align tensor dimensions to wavefront size for better AMD GPU utilization.
        """
        remainder = dim_size % self.wavefront_size
        if remainder != 0:
            padding = self.wavefront_size - remainder
