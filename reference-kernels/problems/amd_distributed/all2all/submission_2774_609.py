...
# ---------------- Packed-All2All optimized impl (修正版) ----------------
    ...
    def combine(self, out_tokens: torch.Tensor, weights: torch.Tensor, expert_meta: torch.Tensor, expert_y: torch.Tensor, expert_num_tokens: torch.Tensor):
        ...
        if total_send == 0:
            ...
            for local_id, cnt in enumerate(counts):
                if cnt == 0:
                    continue
                chunks_meta.append(expert_meta[local_id, :cnt])
                chunks_y.append(expert_y[local_id, :cnt])
            send_meta = torch.cat(chunks_meta, dim=0)  # (total_send, META_DIM)
            send_y = torch.cat(chunks_y, dim=0)         # (total_send, hidden)
