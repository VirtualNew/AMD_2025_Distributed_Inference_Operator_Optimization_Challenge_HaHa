...
# ---------------- Final AMD ROCm Optimized All2All Implementation ----------------
class FinalAMDOptimizedAllToAll:
    ...
    def __init__(self, cfg, rank: int, world_size: int):
        ...
        self.wavefront_size = 64      # AMD wavefront size (64 work-items)
        self.alignment_factor = 128   # For better memory bandwidth utilization
        self.cache_line_size = 64     # AMD cache line size in bytes

    def _align_to_amd_arch(self, size: int, alignment: int = None) -> int:
        """
        Align size to AMD architecture-specific boundaries for optimal performance.
        This helps with memory coalescing and cache utilization on AMD GPUs.
        """
    ...
    def _optimize_tensor_layout(self, tensor: torch.Tensor) -> torch.Tensor:
        ...
        Ensures tensors are contiguous and properly aligned for better performance.
        """
    ...
    # ---------- highly optimized dispatch ----------
    def dispatch(self, dp_x: torch.Tensor, indices: torch.Tensor):
        """
        Highly optimized dispatch implementation for AMD ROCm.
        
        Key optimizations:
        1. Vectorized operations instead of Python loops
        2. Single fused all_to_all_single call for metadata + data
        3. Memory alignment for AMD GPU architectures
        4. Contiguous memory layout for better cache performance
        """
        cfg = self.cfg
        device = dp_x.device
        num_tokens, k = indices.shape

        # AMD optimization: Ensure optimal tensor layout
        dp_x = self._optimize_tensor_layout(dp_x)
        indices = self._optimize_tensor_layout(indices)

        # Vectorized computation of destination information
        # Replaces Python loops with efficient PyTorch operations
        ...
        # Prepare fused send buffer with AMD-specific optimizations
        if total_send == 0:
            ...
            # Create metadata using vectorized stack operations
            k_idx = torch.arange(k, device=device).repeat(num_tokens)[order]
            send_meta = torch.stack([
                ordered_global_eids.int(),                    # global_expert_id
                torch.full_like(ordered_global_eids, self.rank, dtype=torch.int32),  # src_rank
                ordered_token_idx.int(),                      # src_token_idx
                k_idx.int(),                                  # expert_index (k)
                torch.zeros_like(ordered_global_eids, dtype=torch.int32)  # padding
            ], dim=1)
            ...
            # AMD optimization: Ensure packed data is contiguous for better memory access
            send_meta_float = self._optimize_tensor_layout(send_meta_float)
            send_data_float = self._optimize_tensor_layout(send_data_float)
            
            # Fuse metadata and data into a single buffer for optimal transfer
            combined_send = torch.cat([send_meta_float, send_data_float], dim=1)
            combined_send = self._optimize_tensor_layout(combined_send)
        ...
        aligned_total_recv = self._align_to_amd_arch(total_recv, self.alignment_factor)
        recv_combined = torch.empty((aligned_total_recv, per_row_len), dtype=torch.float32, device=device)
        recv_combined = self._optimize_tensor_layout(recv_combined)

        # Single fused all_to_all_single for packed data - key optimization
        # This significantly reduces communication overhead compared to separate transfers
        if total_send > 0 and total_recv > 0:
            dist.all_to_all_single(
                recv_combined[:total_recv],  # Use only the needed portion to minimize memory usage
                combined_send,
        ...
        if total_recv == 0:
            ...
            expert_x = self._optimize_tensor_layout(expert_x)
            expert_meta = self._optimize_tensor_layout(expert_meta)
        ...
        aligned_max_recv = self._align_to_amd_arch(self.max_recv, self.alignment_factor)
    ...
    # ---------- highly optimized combine ----------
    def combine(self, out_tokens: torch.Tensor, weights: torch.Tensor, 
                expert_meta: torch.Tensor, expert_y: torch.Tensor, 
                expert_num_tokens: torch.Tensor):
        """
        Highly optimized combine implementation for AMD ROCm.
        
        Key optimizations:
        1. Vectorized operations instead of Python loops
        2. Single fused all_to_all_single call for metadata + data
        3. Memory alignment for AMD GPU architectures
        4. Efficient scatter_add operations
        """
        cfg = self.cfg
        device = out_tokens.device

        # AMD optimization: Ensure optimal tensor layout for all input tensors
        out_tokens = self._optimize_tensor_layout(out_tokens)
        weights = self._optimize_tensor_layout(weights)
        expert_meta = self._optimize_tensor_layout(expert_meta)
        expert_y = self._optimize_tensor_layout(expert_y)
        expert_num_tokens = self._optimize_tensor_layout(expert_num_tokens)
        ...
        if total_send == 0:
            ...
            if meta_list:
                ...
                send_meta = self._optimize_tensor_layout(send_meta)
                send_y = self._optimize_tensor_layout(send_y)
            else:
                send_meta = torch.empty((0, self.META_DIM), dtype=torch.int32, device=device)
                send_y = torch.empty((0, self.hidden_dim), dtype=cfg.out_dtype, device=device)
        ...
        # Pack send data with AMD-specific optimizations
        if total_send == 0:
            # Handle empty case efficiently
            combined_send = torch.empty((0, per_row_len), dtype=torch.float32, device=device)
        else:
            ...
            ordered_meta_float = self._optimize_tensor_layout(ordered_meta_float)
            ordered_y_float = self._optimize_tensor_layout(ordered_y_float)
        ...
        # Vectorized scatter_add with AMD-specific optimizations
...
def custom_kernel(data: input_t) -> output_t:
    """
    Final optimized custom kernel implementation for AMD ROCm.
    
    This implementation incorporates multiple AMD-specific optimizations:
    1. Memory alignment to wavefront boundaries (64 work-items)
    2. Fused communication operations to reduce overhead
    3. Vectorized operations to minimize Python loops
    4. Contiguous memory layout for better cache performance
    5. Efficient tensor management for AMD GPU architectures
    """
    cfg, rank_data, rank, world_size = data
    torch.cuda.set_device(rank)
    device = torch.device('cuda', rank)
    
    # Create final AMD optimized all-to-all instance
    ata = FinalAMDOptimizedAllToAll(cfg, rank, world_size)

    # Dispatch tokens to experts using highly optimized implementation
    expert_num, expert_x, expert_meta = ata.dispatch(rank_data.x, rank_data.indices)

    # Expert computation (simulated)
    # Use the configured output dtype for better performance on AMD GPUs
    expert_y = expert_x.to(cfg.out_dtype) * (1 + rank)

    # Combine expert outputs back to tokens using highly optimized implementation
